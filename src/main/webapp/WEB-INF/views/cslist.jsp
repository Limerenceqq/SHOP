        ...
        <form id="productForm" action="${path}/image/蔬菜 - Powered By Mango Team.htm" method="get">
            ...
            <div class="pagination">
                ...
                <c:if test="${pageBean.pageNow != 1}">
                    <a href="${path}/product/findProductByCsid?csid=${csid }&pageNow=1" class="firstPage">&nbsp;</a>
                    <a href="${path}/product/findProductByCsid?csid=${csid }&pageNow=${pageBean.pageNow-1}" class="previousPage">&nbsp;</a>
                </c:if>
                <c:forEach var="i" begin="1" end="${pageBean.totalPage}" step="1">
                    ...
                    <c:if test="${pageBean.pageNow!=i}">
                        <a href="${path}/product/findProductByCsid?csid=${csid }&pageNow=${i}">${i}</a>
                    </c:if>
                </c:forEach>

                <c:if test="${pageBean.pageNow != pageBean.totalPage }">
                    <a class="nextPage" href="${path}/product/findProductByCsid?csid=${csid}&pageNow=${pageBean.pageNow+1}">&nbsp;</a>
                    <a class="lastPage" href="${path}/product/findProductByCsid?csid=${csid}&pageNow=${pageBean.totalPage}">&nbsp;</a>
                </c:if>
            </div>
        </form>
    </div>
